...
def plot_enrich(data, n_terms=20, save=False):
    ...
    cbaxes = fig.add_axes([1, 0.15, 0.03, 0.4])
    ...
    #leg = plt.legend([l1, l2, l3, l4], labels, ncol=1, frameon=False, fontsize=12,
                     ...
                     #handletextpad=4, title='Gene overlap', scatterpoints = 1,  bbox_to_anchor=(-2, 1.5), 
                     #facecolor='black')
    leg = plt.legend([l1, l2, l3, l4], labels, ncol=1, frameon=False, fontsize=12,
                     handlelength=1, loc = 'center left', borderpad = 1, labelspacing = 1.4,
                     handletextpad=4, title='Gene overlap', scatterpoints = 1,  bbox_to_anchor=(-1, 1.5), 
                     facecolor='black')
    #plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1)
    plt.tight_layout()
    if save:
        plt.savefig(save, dpi=300, format='pdf', pad_inches=1, bbox_extra_artists=(leg,), bbox_inches='tight')
